...
DATASETS_NAMES = ['imagenet', 'cifar100' , 'cifar10'] ##edited by ffk


def load_data(dataset, data_dir, batch_size, workers, valid_size=0.1, deterministic=False, fine=False):
    """Load a dataset.
    ...
    if dataset == 'cifar100': ##ADDED by ffk
    ...
    if dataset == 'imagenet':
        return imagenet_load_data(data_dir, batch_size, workers, valid_size=valid_size, deterministic=deterministic, fine=fine)
...
def cifar100_load_data(data_dir, batch_size, num_workers, valid_size=0.1, deterministic=False):
    ...
    train_dataset = datasets.CIFAR100(root=data_dir, train=True, ##edited by ffk
    ...
    testset = datasets.CIFAR100(root=data_dir, train=False, ##edited by ffk
...
def finetune_data(data_dir):
    fine_dir = os.path.join(data_dir, 'fine')
    remove_existing = 'rm -v ' + fine_dir + '/*'
    os.system(remove_existing)
    
    train_dir = os.path.join(data_dir, 'train')
    
    classes = [f for f in glob.glob(train_dir + "/**/", recursive=False)]
    
    # random.seed(1)
    fine_classes = random.sample(classes, k=100)
    
    print(len(fine_classes))
    
    for c in fine_classes:
        short = 'ln -s ' + c + ' ' + fine_dir + c[-11:-1]
        # print(short)
        os.system(short)
...
def imagenet_load_data(data_dir, batch_size, num_workers, valid_size=0.1, deterministic=False, fine=False):
    ...
    """
    # if fine:
        # fine_dir = finetune_data(data_dir)
    ...
    num_bandit = len(test_dataset)
    bandices = list(range(num_bandit))
    fraction = int(np.floor(0.1*num_bandit))
    
    ###
    np.random.shuffle(bandices)
    valid_bandice = bandices[:fraction]
    
    bandit_sampler = SubsetRandomSampler(valid_bandice)
    bandit_loader = torch.utils.data.DataLoader(test_dataset,
                                                   batch_size=batch_size, sampler=bandit_sampler,
                                                   num_workers=num_workers, pin_memory=True,
                                                   worker_init_fn=worker_init_fn)
    
    if fine:
        fine_fraction = int(np.floor(0.1*num_train))
        
        ###
        np.random.shuffle(indices)
        fine_indices = indices[:fine_fraction]
        
        fine_sampler = SubsetRandomSampler(fine_indices)
        fine_loader = torch.utils.data.DataLoader(train_dataset,
                                                       batch_size=batch_size, sampler=fine_sampler,
                                                       num_workers=num_workers, pin_memory=True,
                                                       worker_init_fn=worker_init_fn)
        ...
        # new_map = {}
        # for key in fine_dataset.class_to_idx.keys():
            # value_to_be_updated = fine_dataset.class_to_idx[key]
            # new_value = test_dataset.class_to_idx[key]
            # new_map[str(value_to_be_updated)] = new_value
            # fine_dataset.class_to_idx[key] = new_value
        
        # for i in range(len(fine_dataset.samples)):
            # tuple_to_list = list(fine_dataset.samples[i]) 
            # tuple_to_list[1] = new_map[str(fine_dataset.samples[i][1])]
            # fine_dataset.samples[i] = tuple(tuple_to_list)
                
        # fine_dataset.imgs = fine_dataset.samples
        
        # num_fine = len(fine_dataset)
        # fine_indices = list(range(num_fine))
        # fine_sampler = SubsetRandomSampler(fine_indices)
        
        # fine_loader = torch.utils.data.DataLoader(fine_dataset,
            # batch_size=batch_size, sampler=fine_sampler,
            # num_workers=num_workers, pin_memory=True,
            # worker_init_fn=worker_init_fn)
        send_loader = fine_loader
    else:
        send_loader = train_loader
    
    # If validation split was 0 we use the test set as the validation set
    return send_loader, valid_loader or test_loader, test_loader, bandit_loader, input_shape   
